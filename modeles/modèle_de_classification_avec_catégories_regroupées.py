# -*- coding: utf-8 -*-
"""Modèle de classification avec catégories regroupées.ipynb
...
for index, row in df_alim.iterrows():
  if(row["APP_Libelle_activite_etablissement"].__contains__('Poissonnerie')):
    list_categorie.append('Poissonnerie')
    continue
  if(row["APP_Libelle_activite_etablissement"].__contains__('Boucherie')):
    list_categorie.append('Boucherie')
    continue
  if(row["APP_Libelle_activite_etablisse ment"].__contains__('Fermier')):
    list_categorie.append('Fermier')
    continue
  if(row["APP_Libelle_activite_etablissement"].__contains__('Primeur')):
    list_categorie.append('Primeur')
    continue
  if(row["APP_Libelle_activite_etablissement"].__contains__('Fromagerie')):
    list_categorie.append('Fromagerie')
    continue
  if(row["APP_Libelle_activite_etablissement"].__contains__('Restaurant') or row["filtre"].__contains__('Traiteur')):
    list_categorie.append('Restaurant')
    continue
  if(row["APP_Libelle_activite_etablissement"].__contains__('Chocolatier')):
    list_categorie.append('Chocolatier')
    continue
  if(row["APP_Libelle_activite_etablissement"].__contains__('Libre service')):
    list_categorie.append('Libre service')
    continue
  if(row["APP_Libelle_activite_etablissement"].__contains__('glacier')):
    list_categorie.append('glacier')
    continue
  if(row["APP_Libelle_activite_etablissement"].__contains__('Alimentation générale') or row["filtre"].__contains__('Rayon')):
    list_categorie.append('Alimentation générale')
    continue
  #list_categorie.append(row["APP_Libelle_activite_etablissement"])
  list_categorie.append('Autres')

df_alim['categorie'] = list_categorie
df_alim[['categorie','APP_Libelle_activite_etablissement']]
